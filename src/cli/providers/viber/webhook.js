...
export async function setWebhook(_webhook, eventTypes = []) {
  try {
    ...
    const { status } = await client.setWebhook(webhook, eventTypes);

    if (status !== 0) {
      error('Setting for webhook is failed');
    }
...
export async function deleteWebhook() {
  try {
    ...
    const client = ViberClient.connect(accessToken);
    const { status } = await client.removeWebhook();

    if (status !== 0) {
      error('Deleting Viber webhook is failed');
    }
